...
membersuite.priorityPaymentAjaxAPI = (function ($, logger, cardTypeUtil) {
    ...
    var createCustomerVaultToken = function (parms) {
        ...
        $.when(makeRequest(self.options)).then(function () {
            ...
            } else if (!!parms.originalBankAccNumber) {
                return parms.callback({ isSuccessful: true, vaultToken: token.id + '|' + token.bankAccount.token });
            }
        })
        .fail(function () {
    ...
    var createVaultTokenBank = function (parms) {
        var self = this;

        self.options = {
            url: parms.uri + 'vault/bankaccount?token=' + parms.requestToken + '&customerId=' + parms.customerId,
        ...
        $.when(makeRequest(self.options)).then(function () {            
            return parms.callback({ isSuccessful: true, vaultToken: parms.customerId + '|' + token });
        })
        .fail(function () {
            //do whatever work you want on failure
        });
    ...
    return {
        ...
        createVaultTokenBank: createVaultTokenBank
    };
